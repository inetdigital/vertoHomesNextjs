...
export default function HomePageBanner({ singleHomePage, banners }) {
  ...
  const [useRandomImage] = useState(true);
  ...
  useEffect(() => {
    const homePageBannerImages = singleHomePage.data.home_page_banner_images;
    // Generate a random index
    setRandomIndex(Math.floor(Math.random() * homePageBannerImages.length));
  }, []);
  ...
  if (!isBrowser) {
    ...
    return (
      <div className="relative w-full h-screen overflow-hidden">
        {singleHomePage.data?.home_page_banner_images[randomIndex]?.image
          ?.url && (
          <Image
            src={
              singleHomePage.data.home_page_banner_images[randomIndex]?.image
                .url
                ? singleHomePage.data.home_page_banner_images[randomIndex]
                    ?.image.url
                : null
            }
            alt={
